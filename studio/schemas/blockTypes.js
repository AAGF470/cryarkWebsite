...
// ── Raw Diagram Block ─────────────────────────────────────────────────────────
// Renders a self-contained AI-generated Cytoscape.js HTML diagram inside a
// sandboxed iframe via RawDiagramBlock.jsx.
//
// Workflow:
//   1. Hand DIAGRAM_RULES.md + node/edge data to Claude (or any AI).
//   2. It returns a complete self-contained HTML file.
//   3. Paste that HTML into the "Diagram HTML" field below.
//   4. The component renders it with full pan/zoom/drag/mobile support.
export const rawDiagramBlockType = defineType({
  name:  'rawDiagramBlock',
  title: 'Raw Diagram (AI-generated)',
  type:  'object',
  fields: [
    defineField({
      name:        'heading',
    ...
    defineField({
      ...
      type:        'text',
      rows:        20,
      description: 'Paste the complete self-contained HTML generated by AI following DIAGRAM_RULES.md.',
      validation:  R => R.required(),
    }),
    defineField({
      name:         'height',
      title:        'Frame height (px)',
      type:         'number',
      description:  'Height of the diagram iframe in pixels. Default: 560. Use 700–900 for complex graphs.',
      initialValue: 560,
    }),
    defineField({
      name:        'caption',
  ...
  preview: {
    select: { title: 'heading', subtitle: 'html_code' },
    prepare({ title, subtitle }) {
      const snippet = subtitle?.slice(0, 60)?.replace(/\s+/g, ' ')?.trim() ?? ''
      return {
        title:    title ?? 'Raw Diagram',
        subtitle: snippet ? snippet + '…' : 'rawDiagramBlock',
      }
    },
  },
})
...
export const architectureBlockType = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      options: {
        list: [
          { title: 'Compact — small nodes for dense pipelines',   value: 'compact' },
          { title: 'Default — balanced size for most diagrams',   value: 'default' },
          { title: 'Wide — large nodes for spacious layouts',     value: 'wide'    },
        ],
        layout: 'radio',
      },
      initialValue: 'default',
      description: 'Controls the minimum width and text size of every node in the diagram.',
    }),
    defineField({
      name:  'nodes',
      title: 'Nodes',
